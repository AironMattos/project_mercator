...
from schemas import PontoSerieOut, RankingItemOut

router = APIRouter()


@router.get("/ranking/comercio", response_model=list[RankingItemOut])
def ranking_comercio(
    ...
    nunca mostraria. `periodo`: mês de referência (default o último mês
    com cobertura real de INICIO_ATIVIDADE - ver
    servico_indicadores.periodo_padrao_aberturas).
    """
    mes_referencia = periodo or periodo_padrao_aberturas(session)
    itens, sparklines = montar_ranking_aberturas(
