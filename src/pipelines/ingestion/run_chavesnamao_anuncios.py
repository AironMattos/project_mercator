"""Orquestra o conector chavesnamao_anuncios: fetch (lista de URLs de
Curitiba, via sitemap) -> normalize (rate-limited, uma requisição por
página de detalhe) -> grava entidade+observação.

Retomável por design (checkpoint 12d): cada execução consulta quais
identificadores já têm observação para o `observado_em` do dia e pula
essas URLs - rodar de novo depois de uma execução parcial (ou
interrompida) continua de onde parou, sem re-baixar nada. Ver achado de
escala em connector.py (~81 mil anúncios de Curitiba, ~68h pra coletar
tudo a 1 req/3s) - `--limite` existe justamente para isso, uma sessão não
precisa (e não deveria tentar) coletar tudo de uma vez.

Uso:
    python -m pipelines.ingestion.run_chavesnamao_anuncios --limite 500
    python -m pipelines.ingestion.run_chavesnamao_anuncios   # sem limite - roda até esgotar a lista
"""
...
from infrastructure.connectors.chavesnamao_anuncios import (
    ChavesNaMaoAnunciosConnector,
    RegistroNormalizado,
    hash_identificador_anuncio,
)
from infrastructure.connectors.chavesnamao_anuncios.parsing import parse_url_anuncio
...
def _ja_coletados_hoje(fonte_id: str, observado_em: date) -> set[str]:
    ...
    o hash pra cada URL candidata dentro de normalize(), não aqui: esta
    função só devolve os hashes já presentes, quem decide se uma URL
    específica já foi coletada é o loop principal, que tem o id_anuncio
    de cada URL disponível."""
    with get_session() as session:
...
def main() -> None:
    ...
    parser.add_argument("--limite", type=int, default=None, help="máximo de páginas novas a buscar")
    args = parser.parse_args()

    connector = ChavesNaMaoAnunciosConnector()
    ...
    try:
        ...
        logger.info("%d URLs de Curitiba encontradas nos sitemaps", len(snapshot.conteudo))
